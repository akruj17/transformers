...
class BartDecoderLayerParallelAttentions(nn.Module):
    def __init__(self, config: BartConfig):
        super().__init__()
        self.embed_dim = config.d_model
    ...
    def forward(
        ...
        if r_encoder_hidden_states is not None:
            # cross_attn cached key/values tuple is at positions 3,4 of present_key_value tuple
            cross_attn_past_key_value = past_key_value[2:4] if past_key_value is not None else None
            r_result, r_cross_attn_weights, cross_attn_present_key_value = self.r_encoder_attn(
                hidden_states=hidden_states,
                key_value_states=r_encoder_hidden_states,
                attention_mask=r_encoder_attention_mask,
                layer_head_mask=cross_attn_layer_head_mask,
        ...
        if encoder_hidden_states is not None:
            ...
            # cross_attn cached key/values tuple is at positions 5,6 of present_key_value tuple
            cross_attn_past_key_value = past_key_value[4:6] if past_key_value is not None else None
            p_result, p_cross_attn_weights, cross_attn_present_key_value = self.p_encoder_attn(
                hidden_states=hidden_states,
                key_value_states=encoder_hidden_states,
                attention_mask=encoder_attention_mask,
                layer_head_mask=cross_attn_layer_head_mask,
        ...
        # Fully Connected
        hidden_states = residual + p_result + r_result
        residual = hidden_states 
        hidden_states = self.activation_fn(self.fc1(hidden_states))
        hidden_states = nn.functional.dropout(hidden_states, p=self.activation_dropout, training=self.training)
        hidden_states = self.fc2(hidden_states)
...
class BartDoubleEncoderWithCrossAttention(BartPretrainedModel):
    """
    Transformer wrapper that independently encodes two separate input sources, and then finally applies a cross
    attention to both encodings
...
class BartDecoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None,
        ...
        if decoder_type == 'parallel':
            decoder_layer_cls = BartDecoderLayerParallelAttentions
        elif decoder_type == 'serial':
            decoder_layer_cls = BartDecoderLayerSerialAttentions
        else:
            decoder_layer_cls = BartDecoderLayer
